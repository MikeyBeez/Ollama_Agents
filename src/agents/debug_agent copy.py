...
class DebugAgent:
    ...
    def _print_welcome_message(self):
        console.print(f"🚀 {AGENT_NAME} initialized. I'm your advanced debug AI assistant. Type '/q' to quit or '/help' for commands.", style="bold green")
    ...
    def _process_input_core(self, user_input: str) -> str:
        ...
        context = self._gather_enhanced_context(user_input, analysis)
        console.print(Panel(f"Gathered Context: {context[:500]}...", border_style="blue"))

        knowledge = self._retrieve_enhanced_knowledge(context, analysis)
        ...
        causal_relationships = self._perform_causal_analysis(context)

        if "event" in analysis:
            event_causal_chain = self._analyze_event_causal_chain(analysis["event"], context)
        ...
        if response.strip() == self.last_response:
            logger.warning("Generated response is identical to the last response")
            return "I apologize, but I don't have any new information to add. Could you please rephrase your question or ask about a different aspect of the topic?"

        self.last_response = response.strip()

        self._update_conversation_history(user_input, response)
        self._update_knowledge_graph(analysis['topics'], response)
        update_agent_knowledge(response, context, self.config)

        quality_score, quality_explanation = assess_response_quality(response, context, self.config)
